...
if (!file_exists($scriptPath)) {
    echo json_encode([
        "status" => "error",
        "message" => "predict.py tidak ditemukan",
        "path" => $scriptPath
    ]);
    exit;
}

// Escape JSON agar aman di Windows
$jsonInput = str_replace('"', '\"', $jsonInput);

$command = "\"$pythonPath\" \"$scriptPath\" \"$jsonInput\" 2>&1";
...
$predictionReason =
    "Model AI memprediksi status " . $predictionLabel .
    " berdasarkan fitur sensor.";
